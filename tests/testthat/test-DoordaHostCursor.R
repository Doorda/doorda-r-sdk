context('DoordaHostCursor')

source('utilities.R')

test_that('DoordaHostCursor methods work correctly', {
  ...
  cursor <- DoordaHostCursor(post.response)
  ...
  content <- DoordaHostSDK:::response.to.content(r[['response']])
...
test_that('DoordaHostCursor methods work correctly with POST data', {
  ...
  cursor <- DoordaHostCursor(response)
  expect_equal(cursor$infoUri(), '')
  expect_equal(cursor$nextUri(), '')
  expect_equal(cursor$state(), 'FINISHED')
  expect_equal(cursor$fetchedRowCount(), 0)
